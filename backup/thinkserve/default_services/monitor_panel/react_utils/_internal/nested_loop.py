...
_nested_event_loop_policy: NestedLoopPolicy = None  # type: ignore
...
def get_nested_loop(
    ...
    if loop := asyncio._get_running_loop():
        if 'uvloop' in ((type(loop).__module__ or '').lower() + type(loop).__name__.lower()):
...
def patch_asyncio()->bool:
    ...
    if curr_loop and 'uvloop' in ((type(curr_loop).__module__ or '').lower() + type(curr_loop).__name__.lower()):
        _logger.warning('Cannot patch asyncio when there is a running uvloop.')
        return False
