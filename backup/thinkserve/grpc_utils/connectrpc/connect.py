...
from .content_stream import AsyncDataStream
from .error import ConnectError
from .headers import Headers
from .idempotency_level import IdempotencyLevel
from .utils import aiterate, get_acallable_attribute, get_callable_attribute
...
class StreamRequest(RequestCommon, Generic[T]):
...
class UnaryRequest(RequestCommon, Generic[T]):
...
class UnaryResponse(ResponseCommon, Generic[T]):
...
class StreamResponse(ResponseCommon, Generic[T]):
...
async def ensure_single(iterable: AsyncIterable[T], aclose: Callable[[], Awaitable[None]] | None = None) -> T:
...
async def receive_unary_request(conn: StreamingHandlerConn, t: type[T]) -> UnaryRequest[T]:
...
async def receive_stream_request(conn: StreamingHandlerConn, t: type[T]) -> StreamRequest[T]:
...
async def receive_unary_response(
...
async def receive_stream_response(
