        ...
        if is_input_stream:
            ...
            if is_output_stream:
                ...
                if size_of_parameters == 1:
                    ...
                    async def stub_method(  # type: ignore[override]
                        self: object,
...
def is_skip_generation() -> bool:
    ...
    if 'THINKSERVE_RPC_SKIP_GENERATION' in os.environ:
        v = os.getenv("THINKSERVE_RPC_SKIP_GENERATION", "false")
    elif 'PYDANTIC_RPC_SKIP_GENERATION' in os.environ:
        v = os.getenv("PYDANTIC_RPC_SKIP_GENERATION", "false")
    else:
        v = 'false'
    return _to_bool(v, default=False)   # type: ignore

def get_reserved_fields_count() -> int:
    """Get the number of reserved fields to add to each message from environment variable."""
    if 'THINKSERVE_RPC_RESERVED_FIELDS' in os.environ:
        v = os.environ['THINKSERVE_RPC_RESERVED_FIELDS']
    elif 'PYDANTIC_RPC_RESERVED_FIELDS' in os.environ:
        v = os.environ['PYDANTIC_RPC_RESERVED_FIELDS']
    else:
        v = "0"
    try:
        return max(0, int(v))
...
def get_proto_path(proto_filename: str|Path|None = None) -> Path:
    '''
    Get the path to store the .proto file.
    if `proto_filename` is provided, it will be appended to the directory path.
    '''
    from ..common_utils.constants import PROTO_PATH
    if not proto_filename:
        return PROTO_PATH
    return PROTO_PATH / proto_filename
